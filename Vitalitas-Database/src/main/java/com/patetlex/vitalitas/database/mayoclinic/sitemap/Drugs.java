package com.patetlex.vitalitas.database.mayoclinic.sitemap;
...
public class Drugs extends ScrapeableSitemap {
    ...
    public Drugs() {
        super("https://www.mayoclinic.org/patient_consumer_drug.xml");
    }
    ...
    public List<DataEntry> fromJson(String json) {
        return new Gson().fromJson(json, new TypeToken<List<DrugEntry>>() {}.getType());
    }

    protected static class DrugEntry extends DataEntry {
        ...
        public List<TrainingEntry> buildTrainingEntries() {
            ...
            trainingList.add(new TrainingEntry("Add " + this.name + " to my current medications.", "${USER_ADD_MEDICATION-\"" + this.name + "\"}"));
            trainingList.add(new TrainingEntry("Remove " + this.name + " to my current medications.", "${USER_REMOVE_MEDICATION-\"" + this.name + "\"}"));
            if (this.brandNames != null) {
                for (String brandName : this.brandNames) {
                    ...
                    trainingList.add(new TrainingEntry("Add " + brandName + " to my current medications.", "${USER_ADD_MEDICATION-\"" + this.name + "\"}"));
                    trainingList.add(new TrainingEntry("Remove " + brandName + " to my current medications.", "${USER_REMOVE_MEDICATION-\"" + this.name + "\"}"));
                }
            }
            for (String rarity : this.symptoms.keySet()) {
                ...
                for (String symptom : this.symptoms.get(rarity)) {
                    ...
                    i++;
                    trainingList.add(new TrainingEntry("{\"my-medications\":[\"" + this.name + "\"]}\nWhat medication is my symptom " + symptom + " coming from?", this.name + "."));
                    trainingList.add(new TrainingEntry("{\"my-medications\":[\"" + this.name + "\"]}\nShould I be expecting " + symptom + "?", "Yes."));
                }
                symptoms.append(".");
